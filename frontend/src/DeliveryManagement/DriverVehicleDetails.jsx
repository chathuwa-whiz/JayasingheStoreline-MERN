...
const DriverVehicleDetails = () => {
  ...
  const maxDate = new Date(2006, 0, 1) // January 1, 2006
    .toISOString()
    .split('T')[0]; // Convert to yyyy-mm-dd format

  const minDate = new Date(1984, 11, 31) // December 31, 1984
    .toISOString()
    .split('T')[0]; // Convert to yyyy-mm-dd format

  const defaultDate = maxDate; // Set default date to maxDate (2006-01-01)
  ...
  const validateForm = () => {
    ...
    const minBirthDate = new Date('1984-01-01');
    ...
    if (birthDate < minBirthDate || birthDate > maxBirthDate) {
      setMessage({ type: 'error', text: 'Driver must be born between 1984 and 2006' });
      return false;
    }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <input
          type="date"
          id="dob"
          value={newDriver.birthday || defaultDate} // Use defaultDate if no value is set
          min={minDate} // Set min date to 1984-12-31
          max={maxDate} // Set max date to 2006-01-01
